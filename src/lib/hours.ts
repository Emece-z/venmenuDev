...
// completo. Devuelve null si no hay nada aprovechable.
  ...
  for (const d of DAYS) {
    ...
    if (day && typeof day === "object") {
      ...
      week[d.key] = closed
        ...
        : { closed: false, open: coerceTime(dd.open), close: coerceTime(dd.close) };
    }
  }
  return hasMeaningfulHours(week) ? week : null;
}

// Lee los campos `hours-<dia>-closed|open|close` de un FormData.
export function readWeekHoursFromForm(formData: FormData): WeekHours {
  const week = emptyWeek();
  for (const d of DAYS) {
    const closed = formData.get(`hours-${d.key}-closed`) === "on";
    week[d.key] = closed
      ? { closed: true, open: "", close: "" }
      : {
          closed: false,
